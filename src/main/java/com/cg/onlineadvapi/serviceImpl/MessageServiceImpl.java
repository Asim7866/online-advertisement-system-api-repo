...
public class MessageServiceImpl implements MessageService {
	...
	/**
	 * It used to auto-wire MessageRepository bean automatically.
	 */
	@Autowired
	...
	public Message sendMessage(Message message) {
		...
		if (message == null) {
			logger.info("Message Passed is Empty");
			logger.error("Sender is trying to send empty Message");
			throw new NoMessageException("Can't Send Empty Message");
		}
		if (message.getSenderId().equals(message.getReceiverId())) {
			...
			try {
				...
				logger.info("Advertise Doesn't Exist");
				logger.error("Sender with Id " + message.getSenderId()
						+ "is trying to send message for non Existing Advertise");
				throw new NoMessageException("Advertise Id Doesn't Exist");
			}
		}
	}
